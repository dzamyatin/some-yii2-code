<?php
declare(strict_types=1);

namespace App\Shared\User\Infrastructure\Repository;

use App\Shared\User\Domain\Repository\UserTokenRepositoryException;
use App\Shared\User\Domain\Repository\UserTokenRepositoryInterface;
use App\Shared\User\Domain\Entity\User;
use App\Shared\User\Domain\ValueObject\UserToken;
use Firebase\JWT\JWT;
use Firebase\JWT\Key;
use Exception;

class UserTokenRepository implements UserTokenRepositoryInterface
{
    const DEFAULT_PRIVATE_KEY = <<<EOD
    -----BEGIN RSA PRIVATE KEY-----
    
    2MsLL2kHgL8bLTmvXV4FX+PXphrDKg1XxzOYn0otuoqdAQrkK4og
    -----END RSA PRIVATE KEY-----
    EOD;

    const DEFAULT_PUBLIC_KEY = <<<EOD
    -----BEGIN PUBLIC KEY-----
    
    wQIDAQAB
    -----END PUBLIC KEY-----
    EOD;

    const TTL = 86400;

    public function __construct(
        private string $privateKey = self::DEFAULT_PRIVATE_KEY,
        private string $publicKey = self::DEFAULT_PUBLIC_KEY,
    ) {}

    public function makeToken(User $user): string
    {
        $payload = [
            'uid' => $user->getUid(),
            'iat' => time() + self::TTL,
            'nbf' => time(),
        ];

        return JWT::encode($payload, $this->privateKey, 'RS256');
    }

    public function checkRefreshToken(UserToken $token): bool
    {
        //@TODO add refresh token logic
        throw new UserTokenRepositoryException('Refresh is not implemented yet');
    }

    /**
     * @inheritDoc
     */
    public function decodeToken(string $token): UserToken
    {
        try {
            $decoded = JWT::decode(
                $token,
                new Key(
                    $this->publicKey,
                    'RS256'
                )
            );
        } catch (Exception $exception) {
            throw new UserTokenRepositoryException($exception->getMessage(), 0, $exception);
        }

        return new UserToken($decoded->uid ?? '');
    }
}
